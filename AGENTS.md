...
- **The C# is compiled by a local hook, not by CI.** `tools/compile_scripting.py` builds
  ...
  can't cover the C#, and nothing on a runner ever compiles it. There's still no `.csproj`, because
  the compile needs four DLLs from a Qud install and nothing else. If you changed C# and couldn't
  compile it, say so in the pull request.
...
- **Verify claims about Qud against the game's own files** rather than from memory: the installed
  ...
  valid XML. When the claim is that two things are *related*, whether a scope, a tag or an
  `Inherits=`, count what's on the other end of it. Seven of the 36 cultures `Naming.xml` scopes on
  match nothing in the game, `Qudish` among them. And **resolve `Inherits=` before counting anything
  by part**, because a declared-part count is a lower bound, and the tag sentinels `*noinherit` and
  `*delete` mean a naive resolved tag set is wrong in both directions. **And a `Tier` tag says what
  an item costs to make, not when a player meets it.** The population tables decide that, and across
  the tiered armour and weapon tables 36% of entries disagree with their own table's tier. `docs/LESSONS.md` has all three
  worked examples.
