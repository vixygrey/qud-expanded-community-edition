...
This changelog begins at the fork.
Upstream history through 2.2 belongs to Mura.
It remains verbatim in [`docs/2.2-changelog.txt`](docs/2.2-changelog.txt).
That file is a historical record and is never edited.
...
- **(internal)** The charter now lives in `docs/CHARTER.md`, with the six rules, cleared release blockers, and source-document index. `README.md` and `docs/STYLEGUIDE.md` now point to the public charter and no longer depend on private working notes.
- **(internal)** The maintenance lessons now live in `docs/LESSONS.md`, in a public document useful to every contributor and modder.
- **(internal)** The documentation moved to a first-person, warm, direct voice. The convention is recorded in `docs/STYLEGUIDE.md`.
- **(internal)** Repository ignore rules now exclude local agent worktrees, which can otherwise stage a complete duplicate working tree.
- **(internal)** `docs/LESSONS.md` records two option-toggle traps, including cached derived values and restart-scoped requirements.
- **(internal)** `docs/LESSONS.md` records three verification traps from tooling work: a green gate proves only its inspected property, `git checkout <file>` restores from the index, and discovery attributes can fail silently.
- **(internal)** `docs/LESSONS.md` records why a generator must never read the file it writes.
- **(internal)** `CONTRIBUTING.md` and `AGENTS.md` now hold the public workflow and contributor guidance. The project rules no longer depend on private working notes.
- **(internal)** `docs/FEATURES.md` no longer describes already-fixed defects as open.
- **(internal)** The validator now enforces charter rule 5's C# limits and serializable-shape rules directly.
- **(internal)** The fork charter now lives in `docs/CHARTER.md`, with its compatibility, causality, credit, DX, safety, and configurability rules.
  ([#115](https://github.com/vixygrey/qud-expanded-community-edition/issues/115))
...
- **(internal)** `CONTRIBUTING.md` defines the wiki boundary. The wiki explains, while `docs/FEATURES.md` specifies. Figures remain in the feature reference, and the repository documents the checks that protect them.
...
- **(internal)** `docs/LESSONS.md` records why a generator must never read the file it writes. The preview-image generator once read and wrote `mod/preview.png`, which composited the fork's marks twice on later runs.
...
- **(internal)** Three clusters of stale documentation were corrected. The change kept the class-collision account as settled history because it demonstrates that `Class=` is an identifier.
