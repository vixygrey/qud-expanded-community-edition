...
- **Two new validator checks catch silently-ignored XML.** **(internal)** `unknown-part` verifies every `<part Name="…">` resolves to a real class in `XRL.World.Parts`; `dangling-blueprint-ref` verifies that blueprint-valued attributes name a blueprint that exists. Both failures are invisible today — Qud discards a part it cannot resolve and spawns nothing for a blueprint that isn't there, with no error, no log line, and an object that still loads and still validates. #144 shipped with exactly that defect in its history: `GasObject="GasPoison"` names a *part* on the blueprint called `PoisonGas`, and the arrow would have flown, hit, and released no gas.
  ...
  They run **in CI**, which is the part that took the work. Both need names that exist only inside a Caves of Qud install, and GitHub runners have no game and no decompiler. `tools/snapshot_qud_api.py` writes those names to `tools/qud-api.json` — 1,605 part classes and 5,202 blueprints — and that file is committed, so the validator is a plain lookup that runs anywhere. Regenerate it after a Qud update; it records the Steam build it came from, and `--check` compares it against your install without writing. A stale snapshot fails as a false positive on a newly added vanilla name, which is loud, and loud is the correct direction for a tool whose entire purpose is catching silence.
...
- **Boomrose arrows can be crafted.** Vanilla makes no arrow tinkerable — slugs and shotgun shells are craftable, bow ammo is not — so this is a deliberate change to a vanilla item rather than an oversight being corrected, and it is made by merge rather than redeclaration. It costs the same bits as the grenade whose payload it carries and produces **one** arrow, so crafting never beats finding: the wild drops 1d4 at a time, and Qud already gates boomrose by tier, from roughly 1% of the tier-1 ammo pool to about 7% at tier 4. The six arrows above are craftable on exactly those terms, so one rule now covers every effect arrow in the game.
  ([#144](https://github.com/vixygrey/qud-expanded-community-edition/issues/144))
