...
Qud has no hunger and no thirst attrition. Food is buffs; water is currency. That's a
deliberate choice by Freehold — survival timers are busywork, and they cut them. Any
sleep-necessity mod is therefore swimming against the grain of the game's own design,
...
| `Wakeful` | Granted 3–5 rounds after waking from *involuntary* sleep; blocks re-sleep | Respect it. Voluntary sleep should not be blocked by it, involuntary should. |
...
| Sleep gas / Sleep Breath | Involuntary `Asleep` | Should **not** count as restful. Gas-sleep must not clear fatigue. |
...
**Not verified — check these against the game assembly before coding:** the exact class
name of the `Asleep` effect, the `Wakeful` effect, and the API for the telepathy dream
text generator. Generate `Mods.csproj` from the in-game Modding Utilities (needs Mouse
