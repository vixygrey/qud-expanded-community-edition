...
stated, because those are the ones that look arbitrary and are not.
...
not showing up often enough, they are underweighted, and the vanilla cascade is not the problem.
...
skill replaced it, all 24 would be left with no implementation, from Cleave and Berserk! to Tinker I/II/III and
Disassemble, and the mod would be obviously broken rather than subtly wrong. It has
...
> left `23` correct by coincidence, since a power carrying `Class=` does not count toward it, while
...
| **`workshop.json`, `manifest.json`, `preview.png`** | Required names, because the Workshop uploader and Qud's mod loader look for exactly these. |
...
then these were genuinely free, since the fork publishes as a new Workshop item and its notes require a
...
The player's object survives, because its parts are serialised on the object itself, so a weapon still
...
own description, and it doesn't imply the slot belongs to the Psionic Adept genotype, because it's merged
...
**No filename contains a space.** The four that did, namely `Melee Weapons.xml`, `Other Equipment.xml`,
`Psionic Chips.xml` and `Ranged Weapons.xml`, were renamed in #23, because spaces require quoting in
...
- **Chips run a quarter of that curve: `1.25 × 2^tier`**, giving 20 · 40 · 80 · 160 · 320 across tiers
...
- **But a cost vanilla wrote is not always a cost vanilla weighed**, and the two want opposite
  ...
  and **nothing has ever paid one of their costs**, not a player and not a random mutant, so those
...
- **Two-handed variants** get `PenBonus="1"` and a damage bump over the one-handed version, plus
  `UsesTwoSlots="true"`.
- **Two-handed hammer `Physics` weight is the one-handed weight plus 3**, in inventory pounds rather than a
...
- **A tool is found at low tiers and built at high ones.** Where a line is an implement rather than a
  ...
  **This is not a way around `unreachable`.** That check accepts four routes: in a population
  ...
  found disks gated by Tinkering rank. **Bits are `000` plus the item's own tier**, meaning three scrap bits
...
exempt (#373), because a rule nothing has ever followed is not a rule being broken.
...
7, and **1.1x at tier 8**, with Zetachrome Lune at 6,000 sits beside the Flange from the Great Machine
...
ImprovedBlock)` percent, plus 25 for `Shield_Block` and 25 for `Shield_DeftBlocking`, giving 25% bare,
...
because vanilla offers nothing to point at in the Arm slot, since there is no vanilla Arm item above AV 1
...
`Steel Kukri` and `Steel Utility Knife`, though at one tier rather than nine.
...
so clearly: `ArmDagger4` **is** `Dagger4`, with the same `1d8`, the same 75 water and the same penetration, because
...
(6, 8, 10) are on creature limbs, such as cherubic claws and astral tabby bites, not on equipment. This fork's
...
tier.** Fourteen do. `Ironweave Cloak` is the one that demotes rather than extends, from tier 4 to 1,
...
pistol at 100 charge against 10 a turn, or **ten turns of sunlight per shot**. #323 found the solar
...
**700 skill points**, in `Priest of All Suns`. Vanilla's median is 5 and 450. A subtype may exceed the
...
Resistance matters more than the AV, because it is applied as `(100 - resistance) / 100`, so at 100 it
...
**No caste grants cybernetics license points.** Vanilla's genotype is the only source, with zero
...
are 0 by design, because `Math.Min(Bonus, MaxBonus)` means no stat bonus reaches them at all, and fists
are unbounded.
...
`Fullerite Plate Mail` is 160 lb in vanilla against a Strength-16 carry budget of 240, or two thirds
...
vanilla entry, which already holds, and one table's entries are deliberately lighter, so share
...
The ceiling does not apply to a table this fork defines itself, such as `Raven_Chips Tier 1` through `3`
...
- **`Chance` is a list of independent rolls, and their successes are a repeat count.**
  ...
  plus a second half the time, an expectation of 1.5 rather than 1. Vanilla writes a multi-roll chance in
...
are not followed, because a shared sub-table belongs to whoever wrote it. But vanilla's own overgrowth
...
Two *different* blueprints sharing a display name print as one thing twice, as in
...
`<table>` reference down**, via `Population.Generate(Result, Vars, Hint ?? DefaultHint)`, so a nested
...
family's pool. `BaseShield` is nine of ten slices above half and `BaseAxe` eight of nine, which is a rule
...
blueprint resolves. **Every new item should have one**, and the tags below are additions on top, not
...
`DynamicObjectsTable:<Biome>_Creatures` tables look like the route, since 123 vanilla creature blueprints
...
- **Grepping the data.** `Hills_Creatures` really does appear in exactly one file. The names are
  built by string concatenation at runtime, as `"DynamicObjectsTable:" + region + "_Creatures"`, so
...
**Reach for a tag when you want the item in vanilla's specialist pool for its category**, such as the hatter
...
pools is consumed in the `:Tier{n}` form, as in `DynamicObjectsTable:Guns:Tier{zonetier+1}`,
...
defect, because vanilla does it freely, and `Missile 4` carries vanilla's own `Hypertractor` at tier 6.
...
rarity with it.** `Raven_Fine-Tuned Handgun` (tier 6) sits only in `Missile 4`, so no run and no
...
`tag_forms_absent`. Four names vanilla writes both ways, `Fiber`, `Furniture`, `LightSource` and
...
village and dungeon template data, as in `"DynamicSemanticTable:" + text + "::" + villageTechTier` and
...
what Qud mods actually use, and a sample of the installed Workshop mods turned up no CRLF at all,
...
| Line length | No hard limit, because attribute-dense blueprint lines are normal and wrapping them hurts readability |
...
- **Page text is rendered literally.** It sits flush against its tags in vanilla, with
...
- **Parts inherit `IPart` and carry `[Serializable]`.** Keep `WantEvent` lean, because it runs on every
  event fired.
- **Charter rule 5 (safety) is a hard boundary.** No file I/O outside the mod directory, no
  ...
  subscriber, and that is a trust relationship, and those limits do not move.
- **What the C# is allowed to *do* is a separate question, and it is not fixed.** The 36 inert
  ...
  again. Read rule 5 rather than assuming a limit, because this line has been out of date before.
- **Where the game already does a thing in data, use the data**, because a blueprint or a tag is
...
| **A `<book ID>` being the fork's own**, and a book has no `Load`, so touching a vanilla one replaces it | `book-merge-discipline`, `book-duplicate-id`, `book-reference` |
...
| `book-duplicate-id` | `validate_mod.py` | one `<book ID>` per mod, since a repeat clears the first book's pages |
| `book-merge-discipline` | `validate_mod.py` | charter rule 1 for `<book>`, which has no `Load` and so cannot be merged at all |
| `book-reference` | `validate_mod.py` | a `<part Name="Book" ID>` resolving to a book this mod declares, since an unresolved one throws when read |
...
| `codepage-text` | `validate_mod.py` | a C# string literal or JSON value carrying a character Qud's code page 437 render would substitute (XML is exempt, because its reader undoes the map) |
...
| `shader-collision` | `validate_mod.py` | A `<shader>`/`<solidcolor>` name that vanilla already owns, or that this fork declares twice. `Colors.xml` has no `Load` attribute, so merge is the only behaviour and `merge-discipline` cannot see it |
| `helptext-shape` | `validate_mod.py` | An option's `<helptext>` is one line per paragraph and under the length cap. Qud never re-wraps it, and `RTF.FormatToRTF` is called with no `blockWrap`, so every source newline is a forced break in the menu |
