...
| 6 | ✅ Fixed | **`<stag>` used instead of `<tag>`** twice — the advanced hoversled's `Floating` tag and the sphere of negative weight's `Trinket` tag were not being applied, because `<stag>` is not an element Qud reads and an unknown element is skipped in silence. Corrected in #50 (#10); both read `<tag>` today, at `OtherEquipment.xml:140` and `:277`. This row sat at 🟠 Med for the whole time it was fixed, which is what #291 is about. | `ObjectBlueprints/OtherEquipment.xml` lines 140, 277 |
