...
| 7 | ✅ Fixed | **Akimbo reused `Class="Pistol_Akimbo"`** across the Pistol and Multiweapon trees. Renamed to **Two-Gun Stance** and made optional in #11. The shared class is kept deliberately: `Pistol_Akimbo` answers `CanFireAllMissileWeaponsEvent`, a boolean gate, so a separate class would give a double-buyer two identical permissions rather than fixing anything. | `Skills.xml` |
