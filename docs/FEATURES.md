...
| 6 | ✅ Fixed | **`<stag>` used instead of `<tag>`** twice — the advanced hoversled's `Floating` tag and the sphere of negative weight's `Trinket` tag. Changed to `<tag>` in #50 (#10); both read `<tag>` today, at `OtherEquipment.xml:141` and `:278`. This row sat at 🟠 Med for the whole time it was fixed, which is what #291 is about. **The reason recorded here was wrong, and the fix may have been backwards — see #478.** `<stag>` *is* an element Qud reads: `XRL.World.GameObjectFactory` loads `tag` and `stag` into the same dictionary and prefixes the second, so `<stag Name="Floating" />` produces the tag `SemanticFloating`. Vanilla writes both of these names as `<stag>` and nothing else, so its own objects carry `SemanticFloating` and `SemanticTrinket` — which the fork's two no longer do. `tag-form` in `tools/validate_mod.py` now compares against vanilla's usage per name. | `ObjectBlueprints/OtherEquipment.xml` lines 140, 277 |
