...
| 7 | ✅ Fixed | **Akimbo reused `Class="Pistol_Akimbo"`** across the Pistol and Multiweapon trees. Renamed to **Two-Gun Stance** and given its own class `Raven_TwoGunStance` in #11. The shared class was displacing vanilla: `SkillFactory.PowersByClass` holds one entry per class, and the Gunslinger calling grants `<skill Name="Pistol_Akimbo" />` by class, so the mod's entry was being served instead of vanilla's. | `Skills.xml` |
